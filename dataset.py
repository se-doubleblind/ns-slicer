'''Class utilities for data loading, and processing.
'''
import os
import re
import json
...
def remove_comments(string):
    '''Remove in-line and multi-line comments, and empty lines in a code string
    ...
    Returns:
        (str): String with in-line, multi-line comments, and extra lines removed.
    '''
    def replacer(match):
        s = match.group(0)
        if s.startswith('/'):
    ...
    temp = []
    for x in re.sub(pattern, replacer, string).split('\n'):
        if x.strip() != "":
            temp.append(x.strip())
    return '\n'.join(temp)
...
class VDInputExample:
    '''Data structure for vulnerability detection.
    '''
    def __init__(
...
class CompleteDataProcessor(BaseDataProcessor):
    ...
    '''
    def __init__(self, dataset_key, data_dir='../dataset/slice'):
        '''Initializes data processor for complete code examples.

        Arguments:
            dataset_key (str): One of 'Project_CodeNet_Java250', 'gwt'.
            data_dir (str): Path to datasets.
        '''
        if dataset_key not in ['Project_CodeNet_Java250', 'gwt']:
            raise ValueError('Dataset unavailable: Check ``dataset_key``.')
        self.path_to_dataset = Path(data_dir) / dataset_key
...
class PartialDataProcessor(BaseDataProcessor):
    ...
    Parameters:
        ...
        path_to_dataset (pathlib.Path): Path to dataset.
    '''
    def __init__(self, pct, dataset_key, data_dir='../dataset/slice'):
        self.pct = pct
        self.path_to_dataset = Path(data_dir) / dataset_key
