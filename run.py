...
from dataset import CompleteDataProcessor, PartialDataProcessor, AliasingDataProcessor, VulDetectDataProcessor
...
def make_dataloader(args, examples, tokenizer, stage, path_to_dataloader=None):
    '''
    '''
    if stage == 'train':
...
def make_dataloader_vuldetect(args, examples, tokenizer):
    '''
    '''
    batch_size = args.eval_batch_size
    filtered_examples = []
...
if __name__ == '__main__':
    ...
    ## Required parameters
    parser.add_argument("--data_dir", default='../dataset/slice', type=str,
                        help="Path to datasets directory.")
    parser.add_argument("--dataset_key", default='Project_CodeNet_Java250',
                        type=str, choices=['Project_CodeNet_Java250', 'gwt'],
                        help="Dataset type string.")
    ...
    if args.do_train:
        ...
        val_dataloader = make_dataloader(
            args, val_examples, tokenizer, 'val',
...
#        test_examples = cdp.get_val_examples()
        logger.info('Constructing data loader for testing data.')
        test_dataloader = make_dataloader(
            args, test_examples, tokenizer, 'test',
            Path(args.data_dir) / f"dataloader_test.pkl",
        )
#        test_dataloader = make_dataloader(
#            args, test_examples, tokenizer, 'val',
#            Path(args.data_dir) / f"dataloader_val.pkl",
#        )

        stats, label_pairs = evaluate(test_dataloader, model, args)
        print("  Test loss: {0:.4f}".format(stats['Epoch evaluation loss']))
        print("  Backward Slice Accuracy: {0:.4f}".format(stats['BACK']['Accuracy']))
        print("  Backward Slice Precision: {0:.4f}".format(stats['BACK']['Precision']))
        print("  Backward Slice Recall: {0:.4f}".format(stats['BACK']['Recall']))
        print("  Backward Slice F1-Score: {0:.4f}".format(stats['BACK']['F1-Score']))
        print("  Backward Slice EM-Accuracy: {0:.4f}".format(stats['BACK']['EM-Accuracy']))
        print()
        print("  Forward Slice Accuracy: {0:.4f}".format(stats['FORWARD']['Accuracy']))
        print("  Forward Slice Precision: {0:.4f}".format(stats['FORWARD']['Precision']))
        print("  Forward Slice Recall: {0:.4f}".format(stats['FORWARD']['Recall']))
        print("  Forward Slice F1-Score: {0:.4f}".format(stats['FORWARD']['F1-Score']))
        print("  Forward Slice EM-Accuracy: {0:.4f}".format(stats['FORWARD']['EM-Accuracy']))
        print()
        ...
        print("  Overall Slice EM-Accuracy: {0:.4f}".format(stats['OVERALL']['EM-Accuracy']))
    ...
    if args.do_predict:
        ...
        logger.info('Constructing data loader for testing data.')
        test_dataloader, new_examples = make_dataloader_vuldetect(args, examples, tokenizer)
    ...
    if args.do_eval_partial:
        ...
        model.eval()
        # Load test data.
        pdp = PartialDataProcessor(pct=args.pct, dataset_key=args.dataset_key)
        logger.info('Loading test data.')
        test_examples = pdp.get_test_examples()
#        test_examples = pdp.get_val_examples()
        logger.info('Constructing data loader for testing data.')
        test_dataloader = make_dataloader(
            args, test_examples, tokenizer, 'test',
        ...
        # test_dataloader = make_dataloader(
        #     args, test_examples, tokenizer, 'val',
        #     Path(args.data_dir) / f"partial_dataloader_val_{1 - args.pct}.pkl",
        # )

        stats, label_pairs = evaluate(test_dataloader, model, args)
        print("Printing evaluation statistics for Partial Dataset:")
        print("---------------------------------------------------")
        print()
        print("  Test loss: {0:.4f}".format(stats['Epoch evaluation loss']))
        print("  Backward Slice Accuracy: {0:.4f}".format(stats['BACK']['Accuracy']))
    ...
    if args.do_eval_aliasing:
        ...
        model.eval()
        # Load test data.
        adp = AliasingDataProcessor(dataset_key=args.dataset_key)
        logger.info('Loading test data.')
        test_examples = adp.get_test_examples()
#        test_examples = adp.get_val_examples()
        logger.info('Constructing data loader for testing data.')
        test_dataloader = make_dataloader(
            args, test_examples, tokenizer, 'test',
        ...
        # test_dataloader = make_dataloader(
        #     args, test_examples, tokenizer, 'val',
        #     Path(args.data_dir) / f"aliasing_dataloader_val.pkl",
        # )

        stats, label_pairs = evaluate(test_dataloader, model, args)
        print("Printing evaluation statistics for Aliasing Dataset:")
        print("---------------------------------------------------")
        print()
        print("  Test loss: {0:.4f}".format(stats['Epoch evaluation loss']))
        print("  Backward Slice Accuracy: {0:.4f}".format(stats['BACK']['Accuracy']))
